...
export const TaseronKesintiScreen: React.FC<TaseronKesintiScreenProps> = ({
  ...
  /** Dönemdeki tüm taşeronların iş makinesi raporları (toplu yazdır) */
  const makineDonemTumRaporlari = useMemo(() => {
  ...
  }, [taseronKesintiRaporlari, donemAyStr, donemYilStr]);
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white border rounded-2xl p-5 space-y-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <h3 className="text-xs font-black uppercase text-slate-800">
                      Dönem Kesinti Raporları · {ayAdi(selectedAy)} {selectedYil}
                    </h3>
                    <p className="text-[10px] text-slate-500 mt-1">
                      Tüm taşeronlar · Ana Firma Makinesi ve Kiralık ayrı. Saat + iş açıklaması raporda
                      asıl belgedir. Ücret onayı için yukarıdan taşeron seçin.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    <button
                      type="button"
                      disabled={!makineDonemTumRaporlari.length}
                      onClick={() =>
                        yazdirTopluIsMakinesiRaporu(makineDonemTumRaporlari, selectedAy, selectedYil)
                      }
                      className="inline-flex items-center gap-1.5 px-3 py-2 text-[10px] font-black rounded-xl bg-slate-900 text-white disabled:opacity-40 cursor-pointer"
                    >
                      <Printer size={12} /> Toplu Yazdır ({makineDonemTumRaporlari.length})
                    </button>
                    <button
                      type="button"
                      disabled={!makineDonemTumRaporlari.length}
                      onClick={() =>
                        indirTopluIsMakinesiRaporu(makineDonemTumRaporlari, selectedAy, selectedYil)
                      }
                      className="inline-flex items-center gap-1.5 px-3 py-2 text-[10px] font-black rounded-xl border border-slate-300 bg-white text-slate-800 disabled:opacity-40 cursor-pointer"
                    >
                      <Download size={12} /> Toplu İndir
                    </button>
                  </div>
                </div>

                {makineDonemTumRaporlari.length === 0 ? (
                  <p className="text-xs text-slate-400 italic">
                    Bu dönem için henüz iş makinesi kesinti raporu yok. Operatör ekranından veya «Dönem
                    faaliyetlerinden taslak oluştur» ile üretin.
                  </p>
                ) : (
                  makineDonemTumRaporlari.map((r) => {
                    const grup =
                      r.makineKaynakGrup || resolveMakineKaynakGrup(r.faaliyetler?.[0]);
                    const seciliFirma =
                      selectedTaseron &&
                      (r.taseronFirmaId === selectedTaseron.id ||
                        firmaEslesir(r.taseronFirmaAdi, selectedTaseron.unvan));
                    return (
                      <div
                        key={r.id}
                        className={`border rounded-xl p-3 flex flex-wrap gap-3 items-center justify-between ${
                          seciliFirma ? 'border-amber-300 bg-amber-50/40' : 'border-slate-200'
                        }`}
                      >
                        <div className="min-w-0">
                          ...
                          <p className="text-xs font-bold text-slate-800">
                            ...
                            <span className="font-mono text-amber-900">{r.toplamSaat.toFixed(1)} sa</span>
                          </p>
                ...
                {selectedTaseron && makineDonemRaporlari.length > 0 && (
                  <p className="text-[10px] text-amber-800 font-semibold">
                    Seçili taşeron ({selectedTaseron.unvan}): {makineDonemRaporlari.length} rapor vurgulu.
                  </p>
                )}
              </div>
            </div>
