...
async function bootstrapFounderViaServer(emailLower: string, passTrim: string): Promise<void> {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      data.error ||
        'Kurucu hesabı sunucuda senkronize edilemedi. Vercel FIREBASE_SERVICE_ACCOUNT_JSON kontrol edin.'
    );
  }
}
...
async function preparePasswordResetViaServer(emailLower: string): Promise<void> {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      data.error ||
        'Şifre sıfırlama hazırlanamadı. Vercel FIREBASE_SERVICE_ACCOUNT_JSON kontrol edin.'
    );
  }
}
