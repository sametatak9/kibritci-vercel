  ...
  return <div className="max-w-[1500px] mx-auto space-y-4">
    ...
    <section className="rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-950 text-white p-5 sm:p-6 shadow-lg">
      <div className="flex flex-wrap justify-between gap-4">
        ...
        <div className="flex items-center gap-2 self-start bg-white/10 p-2 rounded-2xl border border-white/15"><button onClick={() => setSelectedDate(dateShift(selectedDate, -1))} className="p-2 hover:bg-white/10 rounded-lg"><ChevronLeft size={17}/></button><input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="bg-slate-900 border border-white/20 rounded-lg px-2 py-1.5 text-xs font-bold"/><button onClick={() => setSelectedDate(dateShift(selectedDate, 1))} className="p-2 hover:bg-white/10 rounded-lg"><ChevronRight size={17}/></button><button onClick={printReport} className="px-3 py-2 bg-emerald-400 text-slate-950 rounded-lg text-[10px] font-black uppercase inline-flex gap-1"><FileText size={13}/> Rapor</button></div>
      </div>
    ...
    <section className="grid lg:grid-cols-[1fr_1.2fr] gap-4 items-start">
      ...
      <div className="space-y-3"><div className="flex items-center justify-between px-1"><h2 className="font-black text-slate-800 flex items-center gap-2"><ClipboardList size={18} className="text-emerald-600"/> Günün iş emirleri</h2><span className="text-xs font-bold text-slate-400">{formatDateLabelTr(selectedDate)}</span></div>
        {dayPlans.length === 0 ? <div className="bg-white rounded-2xl border border-dashed p-10 text-center text-slate-400 text-sm">Bu gün için henüz iş planı yok.</div> : dayPlans.map(plan => <article key={plan.id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm space-y-3">
          <div className="flex justify-between gap-3"><div><p className="text-[10px] font-black uppercase text-slate-400 flex gap-1 items-center"><MapPin size={11}/>{plan.parsel} · {plan.blok}</p><h3 className="font-black text-slate-900 mt-1">{plan.isTanimi}</h3><p className="text-xs text-slate-500 mt-1">{plan.personelIds.map(personLabel).join(', ')}</p></div><div className="text-right"><select disabled={busyId === plan.id} value={plan.durum} onChange={(e) => void update(plan, { durum: e.target.value as SahaIsPlanDurum })} className={`text-[10px] font-black border rounded-lg px-2 py-1 ${STATUS[plan.durum].cls}`}>{Object.entries(STATUS).map(([k,v]) => <option key={k} value={k}>{v.label}</option>)}</select><button disabled={busyId === plan.id} onClick={() => void deletePlan(plan)} className="block ml-auto mt-2 text-rose-500"><Trash2 size={15}/></button></div></div>
