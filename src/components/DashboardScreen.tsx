...
import { 
  Users, CalendarCheck2, Tent,
  MapPin, Sparkles, Bell, Building2, ShieldCheck,
} from 'lucide-react';
import {
...
import { DashboardDurumPaneli } from './DashboardDurumPaneli';
                ...
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  {unreadNotifs > 0 && (
                    <span className="inline-flex items-center gap-1 text-[10px] font-bold text-rose-600 bg-rose-50 border border-rose-100 px-2 py-0.5 rounded-full">
                      <Bell size={10} /> {unreadNotifs} okunmamış
                    </span>
                  )}
                </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {kpiCards.map((kpi) => {
            ...
            return (
              <div
                key={kpi.title}
                className={`rounded-2xl bg-white border p-4 shadow-sm ring-1 ${kpi.ring} ${
